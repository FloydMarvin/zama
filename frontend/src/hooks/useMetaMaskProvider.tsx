...
function useMetaMaskInternal(): UseMetaMaskState {
  ...
  const connect = useCallback(() => {
    if (!_currentProvider) {
      console.log("[useMetaMask] No provider available for connection");
      return;
    }
    ...
    console.log("[useMetaMask] Requesting account connection");
  ...
  useEffect(() => {
    ...
    // Cleanup previous provider listeners
    if (prev) {
    ...
    if (next) {
      ...
      // Accounts changed event handler
      ...
      const updateChainId = async () => {
        ...
        try {
          ...
          console.log(
            `[useMetaMask] initialized: chainId=${chainIdHex} accounts.length=${accountsArray.length}`
          );
          ...
          console.log(`[useMetaMask] not connected:`, error);
          _setCurrentProvider(next);
          _setChainId(undefined);
          _setAccounts(undefined);
        }
